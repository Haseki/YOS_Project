...
$error['password2'] = 0;
$error['pass-comp'] = 1;
$email = $username = $password = $entry_time = $password_again = $autho_view = $autho_cons = $autho_change = $autho_create = "";
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($general_error == 0) {
        ...
        if (!isAdminExist($username)) {
            $result = insertAdmin($username, $password, $email, $entry_time, $autho_create, $autho_cons, $autho_change);
            if ($result) {
            ...
            header("location:admin_creating_fail.php?error=2");
        }
    }
